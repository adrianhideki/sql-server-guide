...
### Query JSON
  ...
  In the SQL Server 2016 whe can manage JSON like XML, isn't xquery but have specific commands, let's try some query some JSON:
  ...
  Each row are one whole, delimited by `{}` and the resultset are delimited by a `[]`. With this `json` we can query a json to work like a normal result set. A interesting thing about work with json into sql server is there isn't a specific type like `xml`, whe need store the json into a nvarchar field.
