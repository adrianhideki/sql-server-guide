...
### Errors in the SQL Server
  ...
  In SQL Server we can raise a error with two commands ```RAISERROR``` or ```THROW```.
  ...
  Using the message 50001 to raise a error:
  ```sql
  ...
  # RAISERROR options
  The ```RAIERROR``` have some options that can be informed using ```WITH``` command:
  - ```NOWAIT```
  - ```LOG```
